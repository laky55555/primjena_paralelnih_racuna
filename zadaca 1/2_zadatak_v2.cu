...
__global__ void reduction_jedan_blok(double *in_data, int N, int lda)
{

	__shared__ double smem2[velicina_bloka];

	unsigned int indeks_u_redku = blockIdx.x * blockDim.x * 8 + threadIdx.x;
	unsigned int prvi_indeks_reda = lda * blockIdx.y;
	...
	if(indeks_u_redku < N)
	{
		...
		if(indeks_u_redku + blockDim.x < N && indeks_u_redku + blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + blockDim.x]));
		if(indeks_u_redku + 2*blockDim.x < N && indeks_u_redku + 2*blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 2*blockDim.x]));
		if(indeks_u_redku + 3*blockDim.x < N && indeks_u_redku + 3*blockDim.x != blockIdx.y)	
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 3*blockDim.x]));
		if(indeks_u_redku + 4*blockDim.x < N && indeks_u_redku + 4*blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 4*blockDim.x]));
		if(indeks_u_redku + 5*blockDim.x < N && indeks_u_redku + 5*blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 5*blockDim.x]));
		if(indeks_u_redku + 6*blockDim.x < N && indeks_u_redku + 6*blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 6*blockDim.x]));
		if(indeks_u_redku + 7*blockDim.x < N && indeks_u_redku + 7*blockDim.x != blockIdx.y)
			smem2[threadIdx.x] = fmax(smem2[threadIdx.x], fabs(in_data[ukupni_indeks + 7*blockDim.x]));
		...
		if (threadIdx.x == 0)
		{	
			smem2[0] += = fabs(in_data[prvi_indeks_reda + blockIdx.y]);
...
int main(int argc, char **argv)
{
	...
	grid_size.x = min(((lda + block_size.x - 1) / block_size.x)/8 + 1, 65535);
	grid_size.y = N;
    ...
    {
		case 1024:
			reduction_jedan_blok<1024><<<grid_size, block_size>>>(dev_A_transp, N, lda);
			break;
		case  512:
			reduction_jedan_blok< 512><<<grid_size, block_size>>>(dev_A_transp, N, lda);
			break;
		case  256: 
			reduction_jedan_blok< 256><<<grid_size, block_size>>>(dev_A_transp, N, lda);
			break;
		case  128: 
			reduction_jedan_blok< 128><<<grid_size, block_size>>>(dev_A_transp, N, lda);
			break;
		case   64: 
			reduction_jedan_blok<  64><<<grid_size, block_size>>>(dev_A_transp, N, lda);
			break;
